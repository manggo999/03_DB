...
-- CREATE USER 사용자명 IDENTIFIED BY 비밀번호;
 CREATE USER gmk_sample IDENTIFIED BY 1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한,.... TO 사용자명;
GRANT CREATE SESSION TO gmk_sample;
...
GRANT CREATE TABLE TO gmk_sample;

ALTER USER gmk_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
GRANT CONNECT, RESOURCE TO gmk_sample;
...
SELECT * FROM kh_gmk.EMPLOYEE;
-- SQL Error [942] [42000]: ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) sample 계정의 EMPLOYEE 테이블 조회 권한 부여
--[객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO gmk_sample;
...
REVOKE SELECT ON EMPLOYEE FROM gmk_sample;
