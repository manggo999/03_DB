...
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', ' 남', '2', '여') -- 별칭 사용 X (SELECT 해석이 안되었기때문)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT 절 해석 완료했기 때문)
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPR_CODE로 그룹을 나누고,
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한 사람의 급여가 3백만원 이상이라는 조건(요구사항X)
GROUP BY DEPT_CODE
