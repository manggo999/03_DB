...
FROM EMPLOYEE LEFT OUTER JOIN DEPARTMENT -- EMPLOYEE 기준
ON(DEPT_CODE = DEPT_ID); -- 23GOD(DEPT_CODE가 NULL인 하동운, 이오리 포함)
...
-- 2) RIGHT [OUTER] JOIN : 합치기 사용한 두 테이블 중
-- 오른편에 기술된 테이블의 컬럼 수를 기준으로 JOIN

-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE RIGHT OUTER JOIN DEPARTMENT -- DEPARTMENT 기준
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
-- TIP! 같은 테이블 2개가 있다고 생각하고 JOIN진행
...
SELECT EMP_NAME, DECODE ( SUBSTR(EMP_NO, 8, 1), '2', '여성') 성별,
EMP_NO, DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) 
--연결에 사용할 컬럼명이 다른 경우 ON()을 사용
JOIN JOB USING(JOB_CODE)
