...
-- MOD(숫자 | 칼럼명, 숫자 | 칼럼명) : 나머지 값 반환

-- EMPLOYEE 테이블에서 사원의 월급을 100만원으로 나눴을때 나머지 조회
...
-- 2) 추출한 생년월일을 DATE 타입으로 변경 -> RR 패턴을 이용하여 1900년도로 바꿈
SELECT EMP_NAME,
...
-- NVL(컬럼명, 컬럼값이 NULL일 때 바꿀 값) : NULL인 칼럼값을 다른값으로 변경
...
-- 직원의 성별 구하기
SELECT EMP_NAME,DECODE ( SUBSTR(EMP_NO, 8, 1), '1', '남성', '2', '여성', '3', '남성', '4', '여성') 성별
FROM EMPLOYEE;
...
-- 급여 최소값, 가장 빠른 입사일, 알파벳순서가 가장 빠른 이메일을 조회
SELECT  MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 늦은 입사일, 알파벳순서가 가장 느린 이메일을 조회
...
-- SELECT MAX(SALARY) FROM EMPLOYEE; -- 가장 큰 급여
-- 서브쿼리 + 그룹함수
