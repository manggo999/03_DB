...
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) --SELECT AVG(SALARY) FROM EMPLOYEE = 3047662
ORDER BY JOB_CODE;
-- SELECT 절에 명시되지 않은 칼럼이라도 
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE SALARY = (SELECT MIN(SALARY) FROM EMPLOYEE); --SELECT MIN(SALARY) FROM EMPLOYEE = 1,380,000


-- 노옹철 사원의 급여보다 많은 (초과) 받는 직원의 
-- 사번, 이름, 부서명, 직급명, 급여조회
...
--    > ALL, < ALL : 가장 큰 값 보다 크냐? / 가장 작은 값 보다 작냐?
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '차장'; -- 280만원
...
-- LOCATION 테이블에서 NATIONAL_CODE가 KO인 경우의 LOCAL_CODE가
...
-- 2) DEPARTMENT 테이블 위의 결과(L1)와 동일한 LOCATION_ID를 가지고있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 부서별 입사일이 가장 빠른 사원의 
-- 사원, 이름, 부서코드, 부서명(NULL 이면 '소속없음'), 직급명, 입사일을 조회하고
-- 입사일이 빠른순으로 정렬해라.
-- 단, 퇴사한 직원은 제외해라.

-- 메인쿼리(사원, 이름, 부서코드, 부서명(NULL 이면 '소속없음'), 직급명, 입사일을 조회)
--          (단, 퇴사한 직원은 제외)
...
-- 퇴사자인 이태림을 제외한 상태로, D8 부서의 가장빠른 입사자도
SELECT EMP_ID, EMP_NAME, DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
...
-- 메인쿼리
SELECT EMP_ID, EMP_NAME, NVL(MANAGER_ID, '없음'), MANAGER_NAME
FROM EMPLOYEE;
...
WHERE ROWNUM <= 5; -- 해석순위 2위 WHERE절에서 이제 가상칼럼의 1~5행 까지만 조회
...
/* 1 */ FROM (SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) 평균급여
      FROM EMPLOYEE
      LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_CODE)
...
-- DENSE_RANK() OVER : 동일한 순위 이후의 등수를 이후 순위로 게산
