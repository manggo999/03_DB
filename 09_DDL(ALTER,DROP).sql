...
-- ORA-12899: "KH_GMK"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- > DEPT_ID의 데이터 타입이 CHAR(2) 이므로 영어+숫자 2글자까지 저장 가능
--> D10은 CHAR(3)에 들어올 수 있음
--> VARCHAR(3)으로 변경해보기(남는 바이트 메모리 반환)

-- DEPT_ID 컬럼 테이터 타입 수정
...
SELECT * FROM DEPT_COPY; -- 오류( 왜? 테이블명 변경)
...
DROP TABLE TB1 CASCADE CONSTRAINT; -- 테이블 삭제 시 FK 관계도 모두 삭제
...
-- DML(INSER,UPDATE,DELETE : 데이터(행) 추가/갱신/삭제)
--> DDL은 수행 시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴
--> DDL이 종료 된 후 DML 구문을 수행할 수 있도록 권장!
...
ROLLBACK;
SELECT * FROM TB2; 
-- 롤백 안된다..위에 DDL구문 중 ALTER를 사용해서 그 시점에 COMMIT 됨
